...
Objective pass done mid-project (see chat history for the full reasoning): cross-referencing Bandzoogle's own template taxonomy against Doug's solo site's real structure. Priority order for *this* band specifically (already gigging, so booker-facing content matters more than for a hobbyist): Hero (done) → Music/Listen → Shows (done, dummy data) → **Press/EPK** → About → Photo gallery → Booking/contact. Press/EPK is ranked above About and Gallery here specifically because it's the tool that gets a working band booked, not a generic nice-to-have.
