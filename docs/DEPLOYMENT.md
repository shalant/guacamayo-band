...
- **`import.meta.env.BASE_URL` doesn't end in a slash** on this project (with `base: '/guacamayo-band'` in `astro.config.mjs`) — naively writing `` `${base}favicon.ico` `` silently produces a malformed URL (`/guacamayo-bandfavicon.ico`) that 404s in production while looking fine in a quick local glance. This shipped broken for a bit (favicon + Press download links) before being caught. Always join with an explicit `/` — see the `asset()`/`assetUrl()` helpers in `Layout.astro` and `Press.astro` — and actually check the *rendered* `href`/`src` output (view-source or curl the built HTML), not just that the page looks right.
